...
useSDpatients = 5 # to use only healthy, 1 to use healthy and SDB, 2 to use healthy and NFLE, 3 to use PLM, X to use ins
...
EpochsWork = 1 # number of examined iterations for each subejct, can be used to check multiple classifiers on the same subject
...
# for A phase examination of all subjects cosidered for the leave one out procedure
    ...
    for ff in range (EpochsWork): # repeate the simulation EpochsWork times (number of examine cycles) to attain statistically significant results
        ...
        for a in range (0, len(overlapingSide), 1): # test the three overllaping scenarios if overlapping should be considered or only one examine one classifier if overlapping is not considered (OverLap = 0)
            ...
            if overlapingSide [a] == 0: # classifier for the A phase with overlapping on the right 
                model = Sequential ()
                model.add (Conv1D (128, 5, strides=1, activation = 'relu', input_shape = (features , 1)))
                model.add (MaxPooling1D (pool_size = 2, strides = 2))
                model.add (Dropout (0.1))
                model.add (Conv1D (128*2, 5, activation = 'relu')) 
                model.add (MaxPooling1D (pool_size = 2, strides = 2))
                model.add (Dropout (0.1))
                model.add (Flatten ())
                ...
                model = Sequential ()
                model.add (Conv1D (64, 5, strides=1, activation = 'relu', input_shape = (features , 1)))
                model.add (MaxPooling1D (pool_size = 2, strides = 2))
                model.add (Dropout (0.1))
                model.add (Conv1D (64*2, 5, activation = 'relu')) 
                model.add (MaxPooling1D (pool_size = 2, strides = 2))
                model.add (Dropout (0.1))
                model.add (Conv1D (64*2*2, 5, activation = 'relu')) 
                model.add (MaxPooling1D (pool_size = 2, strides = 2))
                model.add (Dropout (0.1))
                model.add (Flatten ())
                ...
                model = Sequential ()
                model.add (Conv1D (32, 5, strides=1, activation = 'relu', input_shape = (features , 1)))
                model.add (MaxPooling1D (pool_size = 2, strides = 2))
                model.add (Dropout (0.1))
                ...
                model.add (Dense (150, activation = 'relu'))
                model.add (Dense (2, activation = 'softmax'))            
            model = tf.keras.models.load_model(str (disorder) + 'APhase_OverlappingSide_' + str (overlapingSide [a]) + "_subject_" + str(ee) +"_epoch_" + str(ff)) # load the trained model            
            ...
            predictiony_pred = np.zeros (len (YTest)) # variable that will hold the A phase labels predicted by the classifier
            for x in range (len (YTest)): # check all epochs
            ...
            fpr_keras, tpr_keras, thresholds_keras = roc_curve (YTest, proba[:,1]) # produce the receiving operating curve
            ...
            tn, fp, fn, tp = confusion_matrix (YTest, capPredictedPredicted).ravel() # assess the performance of the classifier
            print (classification_report (YTest, capPredictedPredicted))
            ...
            if overlapingSide [a] == 0: # classifier for the NREM with overlapping on the right 
                model = Sequential ()
                model.add (Conv1D (128, 5, strides=1, activation = 'relu', input_shape = (featuresH , 1)))
                model.add (MaxPooling1D (pool_size = 2, strides = 2))
                model.add (Dropout (0.1))
                ...
                model = Sequential ()
                model.add (Conv1D (32, 5, strides=1, activation = 'relu', input_shape = (featuresH , 1)))
                model.add (MaxPooling1D (pool_size = 2, strides = 2))
                model.add (Dropout (0.1))
                model.add (Conv1D (32*2, 5, activation = 'relu')) 
                model.add (MaxPooling1D (pool_size = 2, strides = 2))
                model.add (Dropout (0.1))
                model.add (Conv1D (32*2*2, 5, activation = 'relu')) 
                model.add (MaxPooling1D (pool_size = 2, strides = 2))
                model.add (Dropout (0.1))
                model.add (Conv1D (32*2*2*2, 5, activation = 'relu')) 
                model.add (MaxPooling1D (pool_size = 2, strides = 2))
                model.add (Dropout (0.1))
                model.add (Flatten ())
                model.add (Dense (100, activation = 'relu'))
                ...
                model.add (Dense (150, activation = 'relu'))
                model.add (Dense (2, activation = 'softmax'))
            model = tf.keras.models.load_model(str (disorder) + 'NREM_OverlappingSide_' + str (overlapingSide [a]) + "_subject_" + str(ee) +"_epoch_" + str(ff)) 
            print ("\n\n Testing NREM for subject ", ee, ", epoch ", ff, " and overlapping side ", a)    
            proba2 = model.predict (XTestH)
            predictiony_predh = np.zeros (len (YTesth));
            for x in range (len (YTesth)):
                if proba2 [x, 0] > 0.5:
                    predictiony_predh [x] = 0
                else:
                    predictiony_predh [x] = 1
            tn, fp, fn, tp = confusion_matrix (YTesth, predictiony_predh).ravel()
            fpr_keras, tpr_keras, thresholds_keras = roc_curve (YTesth, proba2 [:, 1])
            ...
            tn, fp, fn, tp = confusion_matrix (YTesth, capPredictedPredictedh).ravel()           
            print (classification_report (YTesth, capPredictedPredictedh))
            ...
            if len(overlapingSide) > 1: # model with overlapping
                if a > 1: # already checked the tree types of overlapping and now it is going for the majority voting to combine the information
                    ...
                    YTest = YTest [OverlappingLeftCorrectL : len (YTest) - OverlappingLeftCorrectR] # alingh the labels, takinh into consideration that they are currently alingn with the overlapping to the left scenario (a = 2)
                    YTest = YTest[CorrectRightA : len (YTest) - CorrectLeftA] # align the A phase and NREM predictions
                    if useSDpatients > 0:
                        StringText = str (disorder) + "EstimatedAPhase_Right_subject{}_Epoch{}_5{}.txt".format(ee, ff, 5) # save the outputs for further examination
                        f = open (StringText, 'ab')
                        pickle.dump (PredictionYA0R, f)
                        f.close ()
                        StringText = str (disorder) + "EstimatedAPhase_Center_subject{}_Epoch{}_5{}.txt".format(ee, ff, 5) 
                        f = open (StringText, 'ab')
                        pickle.dump (PredictionYA1R, f)
                        f.close ()
                        StringText = str (disorder) + "EstimatedAPhase_Left_subject{}_Epoch{}_5{}.txt".format(ee, ff, 5) 
                        f = open (StringText, 'ab')
                        pickle.dump (PredictionYA2R, f)
                        f.close ()
                        StringText = str (disorder) + "EstimatedAPhase_Combined_subject{}_Epoch{}_5{}.txt".format(ee, ff, 5) 
                        f = open (StringText, 'ab')
                        pickle.dump (PredictionYA3, f)
                        f.close ()
                        StringText = str (disorder) + "EstimatedAPhase_CombinationStrategy_subject{}_Epoch{}_5{}.txt".format(ee, ff, 5) 
                        f = open (StringText, 'ab')
                        pickle.dump (cappredictiony_predhA, f)
                        f.close ()
                        StringText = str (disorder) + "DatabaseAPhase_Combined_subject{}_Epoch{}_5{}.txt".format(ee, ff, 5) 
                        f = open (StringText, 'ab')
                        pickle.dump (YTest, f)
                        f.close ()
                    else:
                        StringText = str (disorder) + "EstimatedAPhase_Right_subject{}_Epoch{}-notSDB_5{}.txt".format(ee, ff, 5)  # save the outputs for further examination
                        f = open (StringText, 'ab')
                        pickle.dump (PredictionYA0R, f)
                        f.close ()
                        StringText = str (disorder) + "EstimatedAPhase_Center_subject{}_Epoch{}-notSDB_5{}.txt".format(ee, ff, 5) 
                        f = open (StringText, 'ab')
                        pickle.dump (PredictionYA1R, f)
                        f.close ()
                        StringText = str (disorder) + "EstimatedAPhase_Left_subject{}_Epoch{}-notSDB_5{}.txt".format(ee, ff, 5) 
                        f = open (StringText, 'ab')
                        pickle.dump (PredictionYA2R, f)
                        f.close ()
                        StringText = str (disorder) + "EstimatedAPhase_Combined_subject{}_Epoch{}-notSDB_5{}.txt".format(ee, ff, 5) 
                        f = open (StringText, 'ab')
                        pickle.dump (PredictionYA3, f)
                        f.close ()
                        StringText = str (disorder) + "EstimatedAPhase_CombinationStrategy_subject{}_Epoch{}-notSDB_5{}.txt".format(ee, ff, 5) 
                        f = open (StringText, 'ab')
                        pickle.dump (cappredictiony_predhA, f)
                        f.close ()
                        StringText = str (disorder) + "DatabaseAPhase_Combined_subject{}_Epoch{}-notSDB_5{}.txt".format(ee, ff, 5) 
                        f = open (StringText, 'ab')
                        pickle.dump (YTest, f)
                        f.close ()
                    # for NREM   
                    ...
                    YTesth = YTesth [OverlappingLeftCorrectL : len (YTesth) - OverlappingLeftCorrectR]
                    YTesth = YTesth[CorrectRightH : len (YTesth) - CorrectLeftH] # align the A phase and NREM predictions
                    if useSDpatients > 0:
                        StringText = str (disorder) + "EstimatedNREM_Right_subject{}_Epoch{}_5{}.txt".format(ee, ff, 5)  # save the outputs for further examination
                        f = open (StringText, 'ab')
                        pickle.dump (PredictionYN0R, f)
                        f.close ()
                        StringText = str (disorder) + "EstimatedNREM_Center_subject{}_Epoch{}_5{}.txt".format(ee, ff, 5) 
                        f = open (StringText, 'ab')
                        pickle.dump (PredictionYN1R, f)
                        f.close ()
                        StringText = str (disorder) + "EstimatedNREM_Left_subject{}_Epoch{}_5{}.txt".format(ee, ff, 5) 
                        f = open (StringText, 'ab')
                        pickle.dump (PredictionYN2R, f)
                        f.close ()
                        StringText = str (disorder) + "EstimatedNREM_Combined_subject{}_Epoch{}_5{}.txt".format(ee, ff, 5) 
                        f = open (StringText, 'ab')
                        pickle.dump (PredictionYN3, f)
                        f.close ()
                        StringText = str (disorder) + "EstimatedNREM_CombinationStrategy_subject{}_Epoch{}_5{}.txt".format(ee, ff, 5) 
                        f = open (StringText, 'ab')
                        pickle.dump (cappredictiony_predhN, f)
                        f.close ()
                        StringText = str (disorder) + "DatabaseNREM_Combined_subject{}_Epoch{}_5{}.txt".format(ee, ff, 5) 
                        f = open (StringText, 'ab')
                        pickle.dump (YTesth, f)
                        f.close ()
                    else:
                        StringText = str (disorder) + "EstimatedNREM_Right_subject{}_Epoch{}-notSDB_5{}.txt".format(ee, ff, 5)  # save the outputs for further examination
                        f = open (StringText, 'ab')
                        pickle.dump (PredictionYN0R, f)
                        f.close ()
                        StringText = str (disorder) + "EstimatedNREM_Center_subject{}_Epoch{}-notSDB_5{}.txt".format(ee, ff, 5) 
                        f = open (StringText, 'ab')
                        pickle.dump (PredictionYN1R, f)
                        f.close ()
                        StringText = str (disorder) + "EstimatedNREM_Left_subject{}_Epoch{}-notSDB_5{}.txt".format(ee, ff, 5) 
                        f = open (StringText, 'ab')
                        pickle.dump (PredictionYN2R, f)
                        f.close ()
                        StringText = str (disorder) + "EstimatedNREM_Combined_subject{}_Epoch{}-notSDB_5{}.txt".format(ee, ff, 5) 
                        f = open (StringText, 'ab')
                        pickle.dump (PredictionYN3, f)
                        f.close ()
                        StringText = str (disorder) + "EstimatedNREM_CombinationStrategy_subject{}_Epoch{}-notSDB_5{}.txt".format(ee, ff, 5) 
                        f = open (StringText, 'ab')
                        pickle.dump (cappredictiony_predhN, f)
                        f.close ()
                        StringText = str (disorder) + "DatabaseNREM_Combined_subject{}_Epoch{}-notSDB_5{}.txt".format(ee, ff, 5) 
                        f = open (StringText, 'ab')
                        pickle.dump (YTesth, f)
                        f.close ()
                    ...
                    tn, fp, fn, tp = confusion_matrix (YTest, cappredictiony_predhA, labels=[0,1]).ravel()
                    print (classification_report (YTest, cappredictiony_predhA))
                    accuracy0 = (tp + tn) / (tp + tn + fp + fn)
                    print ('Accuracy : ', accuracy0)
                    sensitivity0 = tp / (tp + fn)
                    print ('Sensitivity : ', sensitivity0)
                    ...
                    if useSDpatients > 0:
                        StringText = str (disorder) + "EstimatedAPhase_PostProcessing_subject{}_Epoch{}_5{}.txt".format(ee, ff, 5) 
                        f = open (StringText, 'ab')
                        pickle.dump (cappredictiony_predhA, f)
                        f.close ()
                    else:
                        StringText = str (disorder) + "EstimatedAPhase_PostProcessing_subject{}_Epoch{}-notSDB_5{}.txt".format(ee, ff, 5) 
                        f = open (StringText, 'ab')
                        pickle.dump (cappredictiony_predhA, f)
                        f.close ()
                    """
                    ...
                    # for i in range (len (ii)): # examine all identified NREM
                    #     if i < len (ii) - 1: # examine untli the last identified NREM
                    #         if jj [i] <= 30: # if the difference is less than 30 then merge the NREM event
                    #             NREMPredicted [ii [i] : ii [i + 1]] = 2 
                    #             counting = counting + 1 
                    #         elif counting >= 2: 
                    #             NREMPredicted [NREMPredicted == 2] = 1 
                    #             counting = 0 
                    #         else: 
                    #             NREMPredicted [NREMPredicted == 2] = 0 
                    #             counting = 0 
                    #     else: 
                    #         NREMPredicted [NREMPredicted == 2] = 0 
                    # NREMPredicted [NREMPredicted == 2] = 0 
                    ("\n\n Testing NREM after ensemble for subject ", ee, ", epoch ", ff)    
                    tn, fp, fn, tp = confusion_matrix (YTesth, NREMPredicted).ravel()
                    print (classification_report (YTesth, NREMPredicted))
                    accuracy0 = (tp + tn) / (tp + tn + fp + fn)
                    print ('Accuracy : ', accuracy0)
                    sensitivity0 = tp / (tp + fn)
                    print ('Sensitivity : ', sensitivity0)
                    ...
                    if useSDpatients > 0:
                        StringText = str (disorder) + "EstimatedNREM_PostProcessing_subject{}_Epoch{}_5{}.txt".format(ee, ff, 5) 
                        f = open (StringText, 'ab')
                        pickle.dump (NREMPredicted, f)
                        f.close ()
                    else:
                        StringText = str (disorder) + "EstimatedNREM_PostProcessing_subject{}_Epoch{}-notSDB_5{}.txt".format(ee, ff, 5) 
                        f = open (StringText, 'ab')
                        pickle.dump (NREMPredicted, f)
                        f.close ()
                ...
                if useSDpatients > 0:
                    StringText = str (disorder) + "EstimatedAPhase_subject{}_Epoch{}_5{}.txt".format(ee, ff, 5) 
                    f = open (StringText, 'ab')
                    pickle.dump (cappredictiony_predhA, f)
                    f.close ()
                    StringText = str (disorder) + "EstimatedNREM_subject{}_Epoch{}_5{}.txt".format(ee, ff, 5) 
                    f = open (StringText, 'ab')
                    pickle.dump (cappredictiony_predhN, f)
                    f.close ()
                else:
                    StringText = str (disorder) + "EstimatedAPhase_subject{}_Epoch{}-notSDB_5{}.txt".format(ee, ff, 5) 
                    f = open (StringText, 'ab')
                    pickle.dump (cappredictiony_predhA, f)
                    f.close ()
                    StringText = str (disorder) + "EstimatedNREM_subject{}_Epoch{}-notSDB_5{}.txt".format(ee, ff, 5) 
                    f = open (StringText, 'ab')
                    pickle.dump (cappredictiony_predhN, f)
                    f.close ()
            """
            ...
            if examineCAP == 1:
                ...
                searchval = 1
                predictedCAP = YTest # variable to be examined by the FSM, holding the A phase locations identified on the databse
                ...
                CAPrateErrototalInter [ff] = (sum (capPredicted) / sum (NREMPredicted)) - (sum (capPredictedDatabase) / sum (YTesth)) # estimate the CAP rate error from the difference between the predicted and the true CAP rate
                CAPrateErroPercentagetotalInter [ff] = (abs ((sum (capPredicted) / sum (NREMPredicted)) - (sum (capPredictedDatabase) / sum (YTesth)))) / (sum (capPredictedDatabase) / sum (YTesth)) # estimate the CAP rate error in percentage
                print('CAP rate error : ', CAPrateErrototalInter [ff])
                print('CAP rate error percentage : ', CAPrateErroPercentagetotalInter [ff])
                if useSDpatients > 0:
                    StringText = str (disorder) + "EstimatedCAP_FSM_subject{}_Epoch{}_5{}.txt".format(ee, ff, 5) 
                    f = open (StringText, 'ab')
                    pickle.dump (capPredicted, f)
                    f.close ()
                    StringText = str (disorder) + "DatabaseCAP_FSM_subject{}_Epoch{}_5{}.txt".format(ee, ff, 5) 
                    f = open (StringText, 'ab')
                    pickle.dump (capPredictedDatabase, f)
                    f.close ()
                else:
                    StringText = str (disorder) + "EstimatedCAP_FSM_subject{}_Epoch{}-notSDB_5{}.txt".format(ee, ff, 5) 
                    f = open (StringText, 'ab')
                    pickle.dump (capPredicted, f)
                    f.close ()
                    StringText = str (disorder) + "DatabaseCAP_FSM_subject{}_Epoch{}-notSDB_5{}.txt".format(ee, ff, 5) 
                    f = open (StringText, 'ab')
                    pickle.dump (capPredictedDatabase, f)
                    f.close ()
